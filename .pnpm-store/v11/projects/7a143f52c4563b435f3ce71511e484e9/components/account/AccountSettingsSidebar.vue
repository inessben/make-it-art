...
const accountNavigation = computed(() => {
  ...
  if (!auth.isAdmin) {
    items.push(buildArtistNavigationItem());
  }
...
function buildArtistNavigationItem() {
  if (auth.isVerifiedArtist) {
    return {
      label: "Artist workspace",
      to: "/artist-profile",
      matches: ["/artist-profile"],
      icon: "M12 3l7 4v10l-7 4-7-4V7l7-4zM9 12l2 2 4-4"
    };
  }
  ...
  if (auth.artistApplicationStatus === "pending") {
    return {
      ...
      icon: "M12 3l7 4v10l-7 4-7-4V7l7-4zM9 12h6M12 9v6"
    };
  }
