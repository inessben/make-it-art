<template>
  <aside
    class="w-full border border-slate-800 bg-slate-950/80 px-5 py-10 lg:min-h-[1115px]"
  >
...
const userInitials = computed(() =>
  getArtistInitials(auth.user?.username || "User"),
);
...
function isActive(item) {
  return item.matches.some(
    (path) =>
      route.path === path ||
      (path === "/orders" && route.path.startsWith("/orders/")),
  );
}
</script>
