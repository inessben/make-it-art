  ...
  <main class="min-h-screen bg-black px-6 py-10 text-[#E6EDF7]">
    ...
    >
      <header class="flex flex-col gap-5 sm:flex-row sm:items-end sm:justify-between">
        <div>
          ...
          <p class="mt-4 max-w-3xl text-sm leading-6 text-[#A0ADB4]">
            Ajoutez / retirez des oeuvres, puis passez au checkout. (Le paiement réel n'est pas encore branché.)
          </p>
        </div>
      ...
      <section v-else class="grid gap-6">
          ...
          >
            <div class="min-w-0">
              ...
              <p class="mt-2 text-sm text-[#A4B0C0]">
                Prix unitaire : {{ formatMarketplacePrice(item.artwork.priceValue ?? item.artwork.price) }}
              </p>
            </div>
        ...
        <div class="grid gap-4 rounded-[24px] border border-[#1A1F2A] bg-[#090017] p-6 sm:grid-cols-[1fr_auto] sm:items-center">
          <div>
