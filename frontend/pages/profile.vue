...
<!-- Pas fan du main si qlq peut le modif svp  -->
  <main class="min-h-screen grid place-items-center px-6 py-10 bg-[#000000] text-[#E6EDF7]">
    <div class="w-full max-w-[1120px] grid gap-7 rounded-[32px] border border-[#1A1F2A] bg-[#01050E] p-7 shadow-[0_32px_90px_rgba(0,0,0,0.22)]">
      <header class="flex flex-col gap-6 sm:flex-row sm:items-start sm:justify-between">
        <div class="max-w-3xl">
          ...
          <h1 class="text-[clamp(2rem,2.5vw,2.8rem)] leading-[1.05] font-semibold">Gérez votre compte et vos préférences</h1>
          <p class="mt-4 max-w-2xl text-[#A0ADB4]">Accédez à vos informations de compte, vos préférences et vos services.</p>
        </div>
      ...
      <section v-if="loading" class="rounded-[24px] border border-[#1A1F2A] bg-[#01050E] p-6 text-[#A0ADB4]">
        <p>Chargement du profil...</p>
      </section>

      <section v-else-if="user" class="flex flex-col gap-6 rounded-[24px] border border-[#1A1F2A] bg-[#01050E] p-6 sm:flex-row sm:items-center sm:justify-between">
        ...
        <span class="inline-flex items-center rounded-full bg-[#4A6CF7]/10 px-4 py-2 text-sm font-semibold text-[#4A6CF7]">Membre</span>
      </section>

      <section v-else class="rounded-[24px] border border-[#1A1F2A] bg-[#01050E] p-6 text-[#A0ADB4]">
        <p>{{ message }}</p>
      </section>
      ...
      <section v-if="user" class="grid gap-4 sm:grid-cols-2 xl:grid-cols-3">
        <article
          v-for="card in cards"
          :key="card.title"
          class="group grid gap-4 rounded-[24px] border border-[#1A1F2A] bg-[#090017] p-5 transition duration-200 hover:-translate-y-1 hover:border-[#262D30]"
        >
          <div class="flex h-12 w-12 items-center justify-center rounded-2xl bg-[#4A6CF7]/10 text-2xl text-[#4A6CF7]">
            {{ card.icon }}
          </div>
          <div>
...
const cards = [
  { icon: "", title: "Paramètres du profil", description: "Mettez à jour vos informations personnelles" },
